...
from pkg_ros_actions.msg import myActionMsgActionResult       # Message Class that is used for Result messages
...
class IotRosBridgeActionClient:
    ...
    def send_goal(self, arg_protocol, arg_mode, arg_topic, arg_message, arg_x, arg_y, arg_theta):
...
class SimpleActionClientTurtle:
    ...
    def send_goal(self, arg_dis, arg_angle):
        ...
        ''' 
	self._sac.send_goal(goal, done_cb=self.done_callback,feedback_cb=self.feedback_callback)
        rospy.loginfo("Goal has been sent.")
    

    
	
    def messagestartcallback(self,condition):
        ...
        if condition.message == "start" :
            
        
        # 3. Send Goals to Draw a hexagon
            obj_client.send_goal(2, 0)
	    rospy.sleep(5)
	    goal_handle1 = action_client.send_goal("mqtt", "pub", action_client._config_mqtt_pub_topic, "final coordinates are",  str(finalx), str(finaly), str(finaltheta))
	    ...
	    goal_handle1 = action_client.send_goal("mqtt", "pub", action_client._config_mqtt_pub_topic, "final coordinates are",  str(finalx), str(finaly), str(finaltheta))
            ...
            print("type start on mqtt pub")

    def done_callback(self, status, result):
        rospy.loginfo("Status is : " + str(status))
        rospy.loginfo("result is :"+ str(result))
	global finalx, finaly, finaltheta
	finalx = result.final_x
	finaly = result.final_y
...
def main():
    ...
    rospy.Subscriber("/ros_iot_bridge/mqtt/sub",msgMqttSub,obj_client.messagestartcallback)
    '''handle_sub_result = rospy.Subscriber('/action_turtle/result', myActionMsgActionResult, obj_client.func_ros_sub_callback)'''
