...
class ur5_1(object):
    ...
    def func_callback_topic_my_topic_color(self,myMsg):
        ...
        od['packagen01']= myMsg.packagen01
    ...
    def for_packagen01(self):
        rospy.logwarn("6. Playing drop to pkgn01 Trajectory File")
        self.moveit_hard_play_planned_path_from_file(self._file_path, 'drop_to_packagen01.yaml', 5)
        self.enableGripper()
        self.moveit_hard_play_planned_path_from_file(self._file_path, 'packagen01_to_drop_with_box.yaml', 5)
	self.disableGripper()
    ...
    def update(self, order_id, city, priority, cost, item,longi,lat):  
        now = datetime.datetime.now()
        date = now.strftime("%d/%m/%y %H:%M:%S")
        parameters = {"id":"OrdersDispatched", "Team Id": "VB#0323","Order ID": order_id,"City" :city, "Item": item, "Priority":priority, "Dispatch Quantity" : '1',"Cost" : cost, "Dispatch Status": "yes", "Dispatch Date and Time" : date, "Longitude" : longi, "Lattitude": lat} 
    ...
    def condition_callback(self, i):
        try:
          ...
          if item_list_a[i][0] == 1:
            ...
            if od['packagen00'] == "red":
                ...
                self.for_packagen32()
                od['packagen32'] = "empty"
            #self.update(item_list_a[i][1],item_list_a[i][2],"HP",item_list_a[i][3],item_list_a[i][4],item_list_a[i][5],item_list_a[i][6])
	    ...
	    self.goal_pub(item_list_a[i][1],item_list_a[i][2],item_list_a[i][4])
            ...
            if od['packagen00'] == "yellow":
                ...
                self.for_packagen32()
                od['packagen32'] = "empty"
            #self.update(item_list_a[i][1],item_list_a[i][2],"MP",item_list_a[i][3],item_list_a[i][4],item_list_a[i][5],item_list_a[i][6])
            #self.update2(item_list_a[i][1],item_list_a[i][2],"HP",item_list_a[i][3],item_list_a[i][4])
	    self.goal_pub(item_list_a[i][1],item_list_a[i][2],item_list_a[i][4])
            item_list_a.pop(i)
            ...
            if od['packagen00'] == "green":
                ...
                self.for_packagen32()
                od['packagen32'] = "empty"
            #self.update(item_list_a[i][1],item_list_a[i][2],"LP",item_list_a[i][3],item_list_a[i][4],item_list_a[i][5],item_list_a[i][6])
            #self.update2(item_list_a[i][1],item_list_a[i][2],"HP",item_list_a[i][3],item_list_a[i][4])
	    self.goal_pub(item_list_a[i][1],item_list_a[i][2],item_list_a[i][4])
            item_list_a.pop(i)
